# Import dependencies
import numpy as np
from Axial_Magnetic_Field.NonHermitian import N_from_center_tight_bind_ham_nonhermitian_hyperbolic_q3 as hyperbolic_strained_hamiltonian
from Axial_Magnetic_Field.reverse import N_from_center_tight_bind_ham_nonhermitian_hyperbolic_q3_Reversed as hyperbolic_strained_hamiltonian_reversed
from Fundamental.Eigenvalue_Degeneracy_Fix import small_chaos as small_disorder
from Fundamental.Haldane import selfconsist_hartreefock_NonHermitian_SublatticeBasis_DisorderAlreadyAdded_SaveRawOutputs_MoreEfficient_CAverage_DiscardReal as SCHF_calculate
from Fundamental.Haldane import selfconsist_hartreefock_Hermitian_SublatticeBasis_DisorderAlreadyAdded_SaveRawOutputs_MoreEfficient_CAverage_DiscardReal as SCHF_calculate_Hermitian
...
hamiltonian = hyperbolic_strained_hamiltonian(pval, nval, alpha_NH, qaxial)
...
if alpha_NH != 0.0:
    ...
    results = SCHF_calculate(pval, nval, hamiltonian_withDisorder,
    ...
    # Perform the SCHF computation
    results = SCHF_calculate_Hermitian(pval, nval, hamiltonian,
                                       initial_guess=0.1, tolerance=schf_tolerance,
                                       hfcoeff_list=np.array([coulomb_V2]))
...
hamiltonian = hyperbolic_strained_hamiltonian_reversed(pval, nval, alpha_NH, qaxial)

if alpha_NH != 0.0:
    hamiltonian_withDisorder = hamiltonian + small_disorder(small_disorder_strength, hamiltonian.shape[0])

    results_reverse = SCHF_calculate(pval, nval, hamiltonian_withDisorder,
                                     initial_guess=0.1, tolerance=schf_tolerance,
                                     hfcoeff_list=np.array([coulomb_V2]))

    eta_values_dict_rev, total_mag_flux_rev, max_real_comp_rev, imag_ev_bounds_rev, smallest_real_ev_rev = results_reverse
else:
    results_reverse = SCHF_calculate_Hermitian(pval, nval, hamiltonian,
                                               initial_guess=0.1, tolerance=schf_tolerance,
                                               hfcoeff_list=np.array([coulomb_V2]))
...
if alpha_NH != 0.0:
    ...
    print('Max imaginary eigval component / Min real eigval component: {}'
          .format(np.max(np.abs(imag_ev_bounds), axis=0) / smallest_real_ev))
